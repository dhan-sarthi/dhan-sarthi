 ...
 * - **`Verify` is inline, not a sticky footer.** Frame 12 puts it under the helper row.
...
export function VerifyOtp({
  ...
  return (
    <Screen header={<Head onBack={onBack} backLabel="Back to the order" title="Verify OTP" />}>
      {/*
       * One block, and `Verify` is inside it rather than pinned to the bottom of the screen.
       *
       * This is the one screen in the spine whose CTA is *not* a sticky bar, and frame 12 is why:
       * the button sits immediately under the helper row, directly above the keypad, as the last
       * step of a five-line sequence — subtitle, cells, countdown, button. The keypad is device
       * chrome and the web does not draw it, so pinning `Verify` to the bottom instead opened
       * 400px of white between the code and the button that confirms it, and separated the two
       * halves of a single act.
       */}
      <div className="pt-2">
