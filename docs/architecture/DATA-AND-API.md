...
| GET | `/api/v1/session` | Session state: cif, asOf, lastSeen, goalTarget, goalBasis, goalKind, caps, scopeOverrides, version, ledgerHorizon, expiresAt, capabilities `{simulatedClock, avatar}`. `goalKind` is the goal the customer chose, null until they choose one. | session bearer | `SessionState` |
