...
| GET | `/api/v1/session` | Session state: cif, asOf, lastSeen, goalTarget, caps, scopeOverrides, version, ledgerHorizon, expiresAt, capabilities `{simulatedClock, avatar}`. | session bearer | `SessionState` |
